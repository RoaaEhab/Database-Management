...
void add_worker()
{
	FILE *textfile = fopen("factorydata.txt", "a");
	if (textfile == NULL)
	{
    ...
    if (0 == size) {
		fprintf(textfile, "First, Last, ID, Salary, Workinghours\n");
    }
	...
	fprintf(textfile, "%s,", wr[WORKER_NUM].fname);
	fprintf(textfile, "%s,", wr[WORKER_NUM].lname);
	fprintf(textfile, "%d,", wr[WORKER_NUM].id);
	fprintf(textfile, "%f,", wr[WORKER_NUM].salary);
	fprintf(textfile, "%d,", wr[WORKER_NUM].working_hours);
...
void readfmFile()
{
	FILE *textfile = fopen("factorydata.txt", "r");
	if (textfile == NULL)
	{
		printf("Error opening file!\n");
		exit(1);
	...
	// Empty the array
	wr = (worker *)malloc(sizeof(worker) * WORKER_NUM);
	...
	while (fgets(line, sizeof(line), textfile))
	{
		...
		printf("%s\n", salary);
		char * working_hours = strtok(NULL, ",");
		printf("%s\n", working_hours);
