...
void add_guest()
{
	FILE *textfile = fopen("hoteldata.txt", "a");
	if (textfile == NULL)
	{
    ...
    if (0 == size) {
		fprintf(textfile, "First, Last, ID, Duration, Totalcost\n");
    }
	...
	fprintf(textfile, "%s,", gs[i].fname);
	fprintf(textfile, "%s,", gs[i].lname);
	fprintf(textfile, "%d,", gs[i].id);
	fprintf(textfile, "%f,", sgst[i].stayDuration);
	fprintf(textfile, "%d,", gs[i].totalCost);
...
void readhmFile()
{
	FILE *textfile = fopen("hoteldata.txt", "r");
	if (textfile == NULL)
	{
		printf("Error opening file!\n");
		exit(1);
	...
	// Empty the array
	gs = (guest *)malloc(sizeof(guest) * GUEST_NUM);
	...
	while (fgets(line, sizeof(line), textfile))
	{
		...
		char * id = strtok(NULL, ",");
		printf("%s\n", id);
		char * stayDuration = strtok(NULL, ",");
		printf("%s\n", stayDuration);
		char * totalCost = strtok(NULL, ",");
		printf("%s\n", totalCost);
